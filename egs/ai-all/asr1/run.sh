...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # 1. AMI
    if [ ! -d ${data_dir-ami}/annotations ]; then
	    local/ami/ami_text_prep.sh ${data_dir-ami}/annotations &
    fi

    # 4. LIBRISPEECH
    for part in dev-clean test-clean dev-other test-other train-clean-100 train-clean-360 train-other-500; do
        # use underscore-separated names in data directories.
        local/librispeech/data_prep.sh ${dwl_dir}/LibriSpeech/${part} {data_dir-librispeech}/${part//-/_} &
    done
    wait # Wait for all process to complete
fi
