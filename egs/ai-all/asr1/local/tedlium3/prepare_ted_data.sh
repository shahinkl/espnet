...
;;'
    # Process the STMs
    cat "${dwl_dir}"/TEDLIUM_release-3/"${data_type}"/${set}/stm/*.stm | sort -k1,1 -k2,2 -k4,4n |
      sed -e 's:<F0_M>:<o,f0,male>:' \
        ...
        -e 's:([^ ]*)$::' |
      awk '{ $2 = "A"; print $0; }'
  } | local/tedlium3/join_suffix.py >"${data_dir}/${set}.orig/stm"
  ...
  cat "${dir}/spk2utt" | awk -v data_type="${data_type}" -v set=${set} -v pwd="${PWD}" '{ printf("%s '"${sph2pipe}"' -f wav -p %s/'"${dwl_dir}"'/TEDLIUM_release-3/%s/%s/sph/%s.sph |\n", $1, pwd, data_type, set, $1); }' >"${dir}/wav.scp"
  cat "${dir}/wav.scp" | awk '{ print $1, $1, "A"; }' >"${dir}/reco2file_and_channel"

  # Create empty 'glm' file
  echo ';; empty.glm
  [FAKE]     =>  %HESITATION     / [ ] __ [ ] ;; hesitation token
  ' >"${data_dir}/${set}.orig/glm"
  ...
  if [ ${set} == "train" ]; then
    mv "${data_dir}/$set.orig/segments" "${data_dir}/$set.orig/segments.temp"
    utils/data/extend_segment_times.py --start-padding=0.15 \
      --end-padding=0.1 <"${data_dir}/${set}.orig/segments.temp" >"${data_dir}/${set}.orig/segments" || exit 1
    rm "${data_dir}/${set}.orig/segments.temp"
  fi
