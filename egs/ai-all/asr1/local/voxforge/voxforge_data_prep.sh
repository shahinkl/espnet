...
command -v flac >/dev/null 2>&1 ||
  {
    echo "FLAC decompressor needed but not found"'!'
    exit 1
  }
...
# make $dwl_dir an absolute pathname.
dwl_dir=$(perl -e '($dir,$pwd)= @ARGV; if($dir!~m:^/:) { $dir = "$pwd/$dir"; } print $dir; ' "$dwl_dir" "${PWD}")

locdata="${data_dir}/local/$lang"
loctmp="${locdata}/tmp"
...
find "${dwl_dir}/" -mindepth 1 -maxdepth 1 |
...
for d in $(find "${dwl_dir}"/ -mindepth 1 -maxdepth 1 -type l -or -type d); do
  basename "${d}"
...
for s in all; do
  ...
  for d in $(cat "${loctmp}/dir_${s}.txt"); do
    ...
    if [ ! -f "${dir}/etc/PROMPTS" ]; then
      echo "No etc/PROMPTS file exists in $dir - skipping the dir ..." >>"${logdir}/make_trans.log"
      continue
    fi
